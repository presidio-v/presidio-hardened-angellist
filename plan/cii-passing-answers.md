...
| `test_most` | **Met** | Measured statement coverage 95.1% and branch coverage 86.0% at v0.7.3. CI enforces both floors independently in the `test` job — statement ≥ 90%, branch ≥ 80% — read from `coverage.json`, because `--cov-fail-under` blends the two metrics into one number. |
