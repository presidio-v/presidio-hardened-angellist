...
`presidio-hardened-angellist` is developed and maintained by **PRESIDIO Group**
...
- **Security-relevant changes** (anything touching the project's
  ...
  The security-sensitive areas are: `hardening.py` (TLS, SSRF guard, secret
  redaction, retry, rate limiting); `intake/email.py` and `intake/csv.py` (the
  untrusted-input parsers and the HTML stripper); `intake/imap.py` and
  `notify.py` (credential handling and transport security); `llm.py` (the
  untrusted-content boundary and the documented `HardenedSession` bypass for
  operator-configured endpoints); `enrich/web.py` (the only fetch of an
  attacker-influenced URL); `config.py` and `rubric_config.py` (fail-closed
  validation of operator input); and `store.py` (SQL construction and the
  on-disk queue).
...
- **Release credentials are not personal.** Publishing uses PyPI **Trusted
  Publishing (OIDC)** bound to the `presidio-v/presidio-hardened-angellist` repository and the
  `pypi` deployment environment, which carries a required-reviewer and
  branch-policy gate — there is no personal API token that dies with an
...
- **The release process is fully documented** — see the tag-verification and
  signing-key instructions in
  [SECURITY.md](SECURITY.md#verifying-releases-and-obtaining-public-signing-keys)
  and the `.github/workflows/publish.yml` workflow — so any authorised
